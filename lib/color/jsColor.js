(function (window) {
	window.jsColorPicker = function(selectors, config) {
		var renderCallback = function(colors, mode) {
			...
			actionCallback = function(event, action) {
				...
				} else if(action === 'changeXYValue' || action === 'changeZValue' || action === 'fromMemory' || action == 'changeOpacityValue'){
					config.onColorChange && config.onColorChange(config.vm);
				}
			},
			...
			doEventListeners = function(elm, multiple, off) {
				var onOff = off ? 'removeEventListener' : 'addEventListener',
					focusListener = function(e) {
						...
						window.ColorPicker.currentInput = input;
						...
						var defaultWidth  = 158,
							...
							top           = position.top +input.offsetHeight + options.margin.top - atrect.top;

						if((left + defaultWidth) > clientWidth){
							left = clientWidth - defaultWidth - 40;
						}

						if(top + defaultHeight > clientHeight){
							top = top - defaultHeight - 30;
						}
	...
	window.docCookies = function(key, val, options) {
